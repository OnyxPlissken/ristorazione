...
import {
  ...
  getControlledOverbookingOpportunity,
...
function formatCustomerPriorityHint(snapshot) {
  if (!snapshot?.band) {
    return "";
  }

  return `Profilo cliente ${snapshot.band} / priorita ${snapshot.priorityScore}.`;
}

function formatDepositNotice(snapshot) {
  if (!snapshot?.depositRequired) {
    return "";
  }

  return snapshot.depositAmount
    ? `Per questa prenotazione e' richiesto un deposito consigliato di EUR ${snapshot.depositAmount.toFixed(2)}. ${snapshot.depositReason}`.trim()
    : snapshot.depositReason || "Per questa prenotazione e' richiesto un deposito consigliato.";
}
  ...
  for (const item of converted) {
    try {
      await createAdminNotification({
        ...
        title: "Coda convertita in prenotazione",
        body: `${item.reservation.guestName} e' stato confermato dalla coda per ${item.location.technicalSettings?.displayName || item.location.name}.`,
        href: `/admin/prenotazioni?reservationId=${item.reservation.id}`
      });
...
export async function createPublicReservationAction(_, formData) {
  ...
  const customerProfile = await ensureCustomerProfile(payload);
  ...
  if (payload.intent === "waitlist") {
    ...
    await createAdminNotification({
      ...
      title: "Nuova richiesta in coda",
      body: `${payload.guestName} ha chiesto ${payload.guests} coperti per ${payload.dateTime.toLocaleString("it-IT")}.`,
      href: "/admin/registro"
    });
    ...
    return buildState({
      ...
      priorityHint: formatCustomerPriorityHint(customerSnapshot)
    });
  }
  ...
  const overbooking = !assignment
    ? await getControlledOverbookingOpportunity({
        location,
        guests: payload.guests,
  ...
  if (!assignment && !overbooking.allowed) {
    return buildState({
      error:
        allowCustomerTableSelection && payload.selectedTableId
  ...
  const reservation = await db.reservation.create({
    data: {
      ...
      dateTime: payload.dateTime,
      notes: payload.notes || null,
      status: assignment ? "CONFERMATA" : "IN_ATTESA",
      source: "PUBBLICA",
      customerBand: customerSnapshot.band,
  ...
  const reservationForNotifications = {
    ...reservation,
    customerProfileId: customerProfile?.id || null,
    depositRequired: customerSnapshot.depositRequired,
    depositAmount: customerSnapshot.depositAmount,
  ...
  await createAuditLog({
    ...
    summary: `Prenotazione pubblica creata su ${assignment?.code || "overbooking controllato"}`,
    metadata: {
      guests: payload.guests,
      tableIds: assignment?.tableIds || [],
      overbooking: Boolean(overbooking.allowed)
    }
  });
  await createAdminNotification({
    locationId: payload.locationId,
    ...
    title: assignment ? "Nuova prenotazione confermata" : "Nuova richiesta in overbooking",
    body: assignment
      ? `${payload.guestName} - ${payload.guests} coperti su ${assignment.code} per ${payload.dateTime.toLocaleString("it-IT")}.`
      : `${payload.guestName} - ${payload.guests} coperti in overbooking controllato per ${payload.dateTime.toLocaleString("it-IT")}.`,
    href: `/admin/prenotazioni?reservationId=${reservation.id}`
  });

  try {
    await sendReservationManageLinkNotification(reservationForNotifications, location);
    if (paymentRequest) {
      await enqueuePaymentLinkNotification(paymentRequest, reservationForNotifications, location);
    }
    await scheduleReservationReminderNotifications(
      reservationForNotifications,
      location,
      customerProfile
    );
  } catch (error) {
  ...
  return buildState({
    success:
      assignment
        ...
        : "Richiesta registrata in overbooking controllato. Il team confermera' il tavolo appena possibile.",
    priorityHint: formatCustomerPriorityHint(customerSnapshot),
    depositNotice: formatDepositNotice(customerSnapshot),
...
export async function updatePublicReservationAction(_, formData) {
  ...
  const previousCustomerProfileId = reservation.customerProfileId;
  const customerProfile = await ensureCustomerProfile(payload);
  let customerSnapshot = buildCustomerSnapshot(
    customerProfile,
  ...
  if (
    ...
    previousCustomerProfileId !== customerProfile?.id
  ) {
    await syncCustomerProfileMetrics(previousCustomerProfileId);
  }
  ...
  await createAdminNotification({
    locationId: reservation.locationId,
    reservationId: reservation.id,
    type: "RESERVATION_UPDATED_BY_CUSTOMER",
    title: "Prenotazione modificata dal cliente",
    body: `${payload.guestName} ha aggiornato la prenotazione a ${payload.dateTime.toLocaleString("it-IT")}.`,
    href: `/admin/prenotazioni?reservationId=${reservation.id}`
  });
  ...
  try {
    ...
    console.error("Errore schedulazione reminder cliente", error);
  }
...
export async function cancelPublicReservationAction(_, formData) {
  ...
  if (reservation.customerProfileId) {
    await syncCustomerProfileMetrics(reservation.customerProfileId);
  }
  ...
  await createAdminNotification({
    ...
    title: "Prenotazione cancellata dal cliente",
    body: `${reservation.guestName} ha cancellato la prenotazione del ${reservation.dateTime.toLocaleString("it-IT")}.`,
    href: "/admin/prenotazioni?status=CANCELLATA"
  });
