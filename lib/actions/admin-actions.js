...
export async function deleteTableAction(formData) {
  ...
  await createAuditLog({
    ...
    summary: `Tavolo eliminato: ${table.code}`,
    metadata: {
      seats: table.seats
    }
  });
...
export async function deleteMenuAction(formData) {
  ...
  await createAuditLog({
    ...
    entityId: menuId,
    action: "MENU_DELETED",
    summary: `Menu eliminato: ${menu.name}`,
    metadata: null
  });
  ...
  return buildReservationActionState({
    success:
      updatedReservation.status === "IN_ATTESA" && requestedStatus === "CONFERMATA" && !tableId
        ...
        : tableIds.length > 1
          ? `Prenotazione aggiornata con successo. Tavoli assegnati: ${tableIds.length}.`
