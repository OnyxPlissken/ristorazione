...
export async function saveYieldFeatureRulesAction(formData) {
  const actor = await requireUser();
  assertPageAccess(actor, "console", "manage");

  const locationId = String(formData.get("locationId") || "");
  const featureSlug = String(formData.get("featureSlug") || "");
  const feature = getYieldFeatureDefinition(featureSlug);

  if (!locationId || !feature) {
    return;
  }
  ...
  const currentRules = normalizeYieldRuleSettings(
    location.technicalSettings?.yieldRuleSettings || {}
  );
  const nextRules = {
    ...currentRules,
    rules: {
  ...
  const validApplyWhen = new Set(YIELD_RULE_APPLY_OPTIONS.map((option) => option.value));

  for (const ruleDefinition of feature.rules) {
    const currentRule = currentRules.rules[ruleDefinition.key];
    const params = {
      ...(currentRule?.params || {})
    };

    for (const control of ruleDefinition.controls) {
      const rawValue = formData.get(`${ruleDefinition.key}.${control.key}`);
      const parsed = Number(rawValue);
      const fallback = control.defaultValue;
      const min = control.min ?? 0;
      const max = control.max ?? 1000;
      params[control.key] = Number.isFinite(parsed)
        ? Math.min(Math.max(parsed, min), max)
        : fallback;
    }

    const applyWhen = String(
      formData.get(`${ruleDefinition.key}.applyWhen`) || currentRule?.applyWhen || ruleDefinition.defaultApplyWhen
    ).toUpperCase();
    const priority = Number(formData.get(`${ruleDefinition.key}.priority`));

    nextRules.rules[ruleDefinition.key] = {
      key: ruleDefinition.key,
      enabled: parseBoolean(formData.get(`${ruleDefinition.key}.enabled`)),
      applyWhen: validApplyWhen.has(applyWhen) ? applyWhen : ruleDefinition.defaultApplyWhen,
      priority: Number.isFinite(priority)
        ? Math.min(Math.max(Math.round(priority), 0), 100)
        : ruleDefinition.defaultPriority,
      params
    };
  }

  const normalizedRules = normalizeYieldRuleSettings(nextRules);
  const technicalPatch = deriveTechnicalSettingsFromYieldRules(
    normalizedRules,
    location.technicalSettings || {}
  );
  const yieldEngineEnabled = parseBoolean(formData.get("yieldEngineEnabled"));

  await db.locationTechnicalSetting.upsert({
    where: {
      locationId
    },
    create: {
      locationId,
      yieldEngineEnabled,
      yieldRuleSettings: normalizedRules,
      ...technicalPatch
    },
    update: {
      yieldEngineEnabled,
  ...
  await createAuditLog({
    userId: actor.id,
    locationId,
    entityType: "yield_rules",
    entityId: locationId,
    action: "YIELD_RULES_UPDATED",
    summary: `Regole ${feature.title} aggiornate`,
    metadata: {
      featureSlug,
      yieldEngineEnabled,
      rules: feature.rules.map((rule) => ({
        key: rule.key,
        enabled: normalizedRules.rules[rule.key]?.enabled,
        applyWhen: normalizedRules.rules[rule.key]?.applyWhen
      }))
    }
  });

  revalidateAdminSection("/admin/console");
  revalidateAdminSection(`/admin/console/funzioni/${featureSlug}`);
  revalidateAdminSection("/admin/sala");
  revalidateAdminSection("/admin/analytics");
  revalidatePath("/prenota");
}
