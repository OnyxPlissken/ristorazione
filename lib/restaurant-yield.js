...
export function predictReservationDurationMinutes(
  ...
  if (!technicalSettings?.yieldEngineEnabled || technicalSettings?.predictiveDurationEnabled === false) {
    return baseDuration;
  }
...
export function buildKitchenLoadBuckets(reservations = [], technicalSettings = {}) {
  const windowMinutes = technicalSettings.kitchenLoadWindowMinutes || 30;
  const maxCovers = Math.max(Number(technicalSettings.kitchenLoadMaxCovers || 0), 0);
...
export function canAddCoversToKitchenLoad({
  ...
  if (!technicalSettings.yieldEngineEnabled || technicalSettings.kitchenLoadGuardEnabled === false) {
    return {
      allowed: true,
      covers: 0,
      maxCovers: Number(technicalSettings.kitchenLoadMaxCovers || 0),
      label: ""
    };
  }

  const maxCovers = Number(technicalSettings.kitchenLoadMaxCovers || 0);
  ...
  const bucket = getKitchenLoadBucket(
    dateTime,
    technicalSettings.kitchenLoadWindowMinutes || 30
  );
  const bucketKey = bucket.toISOString();
  const buckets = buildKitchenLoadBuckets(reservations, technicalSettings);
...
export function getControlledOverbookingDecision({
  ...
  if (!technicalSettings.yieldEngineEnabled || !technicalSettings.controlledOverbookingEnabled) {
    return {
      allowed: false,
  ...
  const maxExtraCovers = Number(technicalSettings.controlledOverbookingMaxCovers || 0);
  ...
  const minReliability = Number(technicalSettings.controlledOverbookingMinReliabilityScore || 0);
