...
import {
  compareReservationAssignmentCandidates,
  formatAssignmentReason,
  getTableAssignmentPolicy,
  scoreReservationAssignment
} from "./table-assignment-policy";
import {
  canAddCoversToKitchenLoad,
  getControlledOverbookingDecision
...
function getEffectiveDurationMinutes(durationMinutes, policy) {
  return durationMinutes + (policy?.turnoverBufferMinutes || 0);
}
...
function findCombinedTableAssignment(availableTables, guests, policy = getTableAssignmentPolicy()) {
  const lookup = buildTableLookup(availableTables);
  const candidates = [];
  const maxCombinationSize = policy.maxTables || MAX_COMBINATION_SIZE;
  ...
  function expand(currentTables, remainingTables) {
    ...
    if (currentTables.length >= maxCombinationSize) {
      return;
    }
...
function findAssignmentFromAvailableTables(
  availableTables,
  guests,
  policy = getTableAssignmentPolicy()
) {
  ...
  if (!policy.combineTablesEnabled) {
    return null;
  }

  return findCombinedTableAssignment(availableTables, guests, policy);
}
...
function buildUnavailableSlotResult(slot, reason, fitLabel = "Non disponibile") {
  return {
    ...slot,
    available: false,
    availableTableCount: 0,
...
export async function findAssignableTables(
  ...
  const policy = getTableAssignmentPolicy(technicalSettings);
  const effectiveDurationMinutes = getEffectiveDurationMinutes(durationMinutes, policy);
  const tables = await getCandidateTables(locationId);
  ...
  const { start, end } = getReservationWindow(dateTime, effectiveDurationMinutes);
  const reservations = await getBlockingReservations({
    locationId,
    start,
    end,
  ...
  return findAssignmentFromAvailableTables(availableTables, guests, policy);
}
...
export async function getSlotAvailability({
  ...
  const effectiveDurationMinutes = getEffectiveDurationMinutes(
  ...
  const slotResults = await Promise.all(
    slots.map(async (slot) => {
      const window = getReservationWindow(slot.dateTime, effectiveDurationMinutes);
      const occupiedTableIds = getOccupiedTableIds({
        reservations,
        start: window.start,
        end: window.end,
      ...
      if (!kitchenLoad.allowed) {
        return buildUnavailableSlotResult(
          slot,
          `Carico cucina saturo alle ${kitchenLoad.label}: ${kitchenLoad.nextCovers}/${kitchenLoad.maxCovers} coperti.`,
          "Cucina satura"
        );
      }

      const availableTables = tables.filter((table) => !occupiedTableIds.has(table.id));
      const assignment = findAssignmentFromAvailableTables(availableTables, guests, policy);

      const result = buildSlotAvailabilityResult({
        slot,
        tables,
        availableTables,
        assignment,
      ...
      if (!result.available) {
        const overbooking = getControlledOverbookingDecision({
          reservations,
          dateTime: slot.dateTime,
          guests,
          technicalSettings: location.technicalSettings || {}
        ...
        if (overbooking.allowed) {
          return {
            ...result,
            available: true,
            fitLabel: "Overbooking controllato",
            recommendationReason: overbooking.reason,
            tableLabel: "Assegnazione da confermare",
            slotScore: slotOptimizationEnabled ? 42 : 0
          };
        }
      }
...
export async function getControlledOverbookingOpportunity({
  location,
  guests,
  dateTime,
  ...
  if (!location?.id) {
    return {
      allowed: false,
      reason: "Sede non valida."
    };
  }

  const dayStart = parseDateInput(getDateKey(dateTime));
  const dayEnd = new Date(dayStart);
  dayEnd.setHours(23, 59, 59, 999);
  ...
  return getControlledOverbookingDecision({
    reservations,
    dateTime,
    guests,
    customerProfile,
...
function buildReservationCandidateContext(reservations) {
  const timestamps = reservations
    .map((reservation) => new Date(reservation.createdAt || Date.now()).getTime())
    .filter(Number.isFinite)
    .sort((left, right) => left - right);

  return {
    oldestCreatedAt: timestamps[0] ? new Date(timestamps[0]) : null,
    newestCreatedAt: timestamps[timestamps.length - 1]
      ? new Date(timestamps[timestamps.length - 1])
      : null
  };
}

export function buildReservationAssignmentCandidates({
  reservations,
  availableTables,
  policy = getTableAssignmentPolicy()
}) {
  const context = buildReservationCandidateContext(reservations);

  return reservations
    .map((reservation) => {
      const assignment = findAssignmentFromAvailableTables(
        availableTables,
        reservation.guests,
        policy
      );
      ...
      const score = scoreReservationAssignment({
        reservation,
        assignment,
        policy,
        context
      ...
      return {
        ...
        assignment,
        score,
        reason: formatAssignmentReason({ reservation, assignment, score })
      };
    })
    .filter(Boolean)
    .sort(compareReservationAssignmentCandidates);
}
