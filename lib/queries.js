...
import {
  buildKitchenLoadBuckets,
  buildOwnerBrief,
  getRevenuePerSeatHour,
  predictReservationDurationMinutes
} from "./restaurant-yield";
...
function withLocationScope(where, locationId, field = "locationId") {
  if (!locationId) {
    return where;
  }

  const currentScope = where?.[field];

  if (
    currentScope &&
    typeof currentScope === "object" &&
    Array.isArray(currentScope.in) &&
    !currentScope.in.includes(locationId)
  ) {
    return {
      ...where,
      [field]: {
...
export async function getAdminDashboardData(user, options = {}) {
  const activeLocationId = options.locationId || "";
  const whereLocation = withLocationScope(locationFilter(user), activeLocationId, "id");
  ...
  const [
    ...
    db.diningTable.count({
      where: {
        ...(activeLocationId ? { locationId: activeLocationId } : { locationId: { in: locationIds } }),
        archivedAt: null
      }
    }),
    ...
    db.customerProfile.count({
      where: {
        ...
        band: "A",
        reservations: {
          some: withLocationScope(reservationFilter(user), activeLocationId)
        }
      }
    })
  ]);
...
export async function getReservationsPageData(user, options = {}) {
  const activeLocationId = options.locationId || "";
  const reservations = await db.reservation.findMany({
    where: withLocationScope(reservationFilter(user), activeLocationId),
    include: {
      location: {
        include: {
          technicalSettings: true,
...
export async function getAdminReservationLiveSummary(user, options = {}) {
  const where = withLocationScope(reservationFilter(user), options.locationId || "");
...
export async function getFloorPageData(user, options = {}) {
  ...
  let normalizedReservations = reservations.map((reservation) => {
  ...
  const assignmentPolicy = getTableAssignmentPolicy(selectedLocation.technicalSettings || {});
  const engineCandidates = assignmentPolicy.enabled
    ? buildReservationAssignmentCandidates({
        ...
        availableTables: selectedLocation.tables.filter((table) => table.active && !table.archivedAt),
        policy: assignmentPolicy
      })
    : [];
  const engineCandidateByReservationId = new Map(
    engineCandidates.map((candidate, index) => [
      candidate.reservation.id,
      {
        engineRank: index + 1,
        engineScore: candidate.score.total,
        engineReason: candidate.reason,
        recommendedTableCodes: candidate.assignment.tables.map((table) => table.code)
      }
    ])
  );
  ...
  normalizedReservations = normalizedReservations.map((reservation) => ({
    ...reservation,
    ...(engineCandidateByReservationId.get(reservation.id) || {})
  }));
  ...
  return {
    ...
    kitchenLoad: buildKitchenLoadBuckets(
      normalizedReservations,
      selectedLocation.technicalSettings || {}
    )
  };
}
...
export async function getAnalyticsPageData(user, options = {}) {
  const activeLocationId = options.locationId || "";
  const reservationWhere = withLocationScope(reservationFilter(user), activeLocationId);
  const waitlistWhere = withLocationScope(waitlistFilter(user), activeLocationId);
  const posWhere = activeLocationId
    ? { locationId: activeLocationId }
    : scopedWhere(user, "locationId");
  const [locations, reservations, waitlistEntries, customerProfiles, posTransactions] = await Promise.all([
    db.location.findMany({
      where: withLocationScope(locationFilter(user), activeLocationId, "id"),
      include: {
        settings: true,
        technicalSettings: true,
        tables: {
  ...
  const tableLookup = new Map(
    locations.flatMap((location) =>
      (location.tables || []).map((table) => [table.id, table])
    )
  );
  const completedYieldReservations = completedReservations.filter(
    (reservation) => numberValue(reservation.spendAmount) > 0
  );
  const revenuePerSeatHourValues = completedYieldReservations.map((reservation) => {
    const location = locationById.get(reservation.locationId) || {};
    return getRevenuePerSeatHour({
      reservation,
      tableLookup,
  ...
  const revenuePerSeatHour =
    revenuePerSeatHourValues.length > 0
      ? revenuePerSeatHourValues.reduce((sum, value) => sum + value, 0) /
        revenuePerSeatHourValues.length
      : 0;
  const predictedDurations = recentReservations.map((reservation) => {
    const location = locationById.get(reservation.locationId) || {};
    return predictReservationDurationMinutes(
      reservation,
      location.settings || {},
  ...
  const avgPredictedDuration =
    predictedDurations.length > 0
      ? Math.round(
          predictedDurations.reduce((sum, value) => sum + value, 0) /
  ...
  for (const reservation of last30Reservations) {
    ...
    if (reservation.status === "COMPLETATA") {
      ...
      const currentTable = revenueByTableMap.get(tableCode) || {
        ...
        revenue: 0,
        reservations: 0,
        revenuePerSeatHour: 0,
        yieldSamples: 0
      };

      currentTable.revenue += numberValue(reservation.spendAmount);
      currentTable.reservations += 1;
      currentTable.revenuePerSeatHour += getRevenuePerSeatHour({
        reservation,
        tableLookup,
        locationSettings: location.settings || {},
        technicalSettings: location.technicalSettings || {}
      });
      currentTable.yieldSamples += numberValue(reservation.spendAmount) > 0 ? 1 : 0;
  ...
  const activeTechnicalSettings =
    activeLocationId && locations.length === 1
      ? locations[0].technicalSettings || {}
      : locations[0]?.technicalSettings || {};
  const kitchenLoad = buildKitchenLoadBuckets(last30Reservations, activeTechnicalSettings);
  ...
  return {
    stats: {
      ...
      peakKitchenCovers: peakKitchenLoad?.covers || 0,
      peakKitchenLabel: peakKitchenLoad?.label || "",
      peakKitchenLoadPercent: peakKitchenLoad?.loadPercent || 0,
    ...
    tableRevenue: [...revenueByTableMap.values()]
      .map((table) => ({
        ...table,
        revenuePerSeatHour:
          table.yieldSamples > 0 ? table.revenuePerSeatHour / table.yieldSamples : 0
      }))
    ...
    kitchenLoad: kitchenLoad.slice(0, 12),
    ownerBrief
  };
}

export async function getCustomerCrmPageData(user, options = {}) {
  const activeLocationId = options.locationId || "";
  const reservationWhere = withLocationScope(reservationFilter(user), activeLocationId);
  const waitlistWhere = withLocationScope(waitlistFilter(user), activeLocationId);
